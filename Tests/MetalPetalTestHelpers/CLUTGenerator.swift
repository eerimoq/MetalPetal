...
public struct CLUTImageLayout {
    public var horizontalTileCount: Int
    public var verticalTileCount: Int
    public init(horizontalTileCount: Int, verticalTileCount: Int) {
...
public struct CLUTImageDescriptor {
    ...
    public var isValid: Bool {
        dimension > 0 && dimension <= 256 &&
            (layout.horizontalTileCount * layout.verticalTileCount == dimension)
    }

    public var title: String {
        "CLUT_D\(dimension)_\(layout.horizontalTileCount * dimension)x\(layout.verticalTileCount * dimension)"
    }

    public static let squareD16: CLUTImageDescriptor = .init(
        dimension: 16,
        layout: CLUTImageLayout(horizontalTileCount: 4, verticalTileCount: 4)
    )

    public static let squareD64: CLUTImageDescriptor = .init(
        dimension: 64,
        layout: CLUTImageLayout(horizontalTileCount: 8, verticalTileCount: 8)
    )

    public static let squareD256: CLUTImageDescriptor = .init(
        dimension: 256,
        layout: CLUTImageLayout(horizontalTileCount: 16, verticalTileCount: 16)
    )

    public init(dimension: Int, layout: CLUTImageLayout) {
        self.dimension = dimension
        self.layout = layout
    }
}

public enum IdentityCLUTImageGenerator {
    public static func generateIdentityCLUTImage(with descriptor: CLUTImageDescriptor) -> CGImage? {
        ...
        guard let dataProvider = CGDataProvider(data: CFDataCreate(
            kCFAllocatorDefault,
            buffer.assumingMemoryBound(to: UInt8.self),
            imageHeight * bytesPerRow
        )) else {
            return nil
        }
        return CGImage(width: imageWidth,
                       ...
                       bitmapInfo: CGBitmapInfo(rawValue: CGBitmapInfo.byteOrder32Little
