...
template <typename T,
          typename _E = typename enable_if<is_floating_point<typename make_scalar<T>::type>::value>::type>
...
METAL_FUNC float lum(float4 C) { return 0.299 * C.r + 0.587 * C.g + 0.114 * C.b; }
...
{
    float4 B = float4(overlayBlendSingleChannel(Cb.r, Cs.r), overlayBlendSingleChannel(Cb.g, Cs.g),
                      overlayBlendSingleChannel(Cb.b, Cs.b), Cs.a);
    return blendBaseAlpha(Cb, Cs, B);
}

// hardLight
METAL_FUNC float4 hardLightBlend(float4 Cb, float4 Cs) { return overlayBlend(Cs, Cb); }
...
{
    return s < 0.5 ? (b - (1 - 2 * s) * b * (1 - b))
                   : (b + (2 * s - 1) * (softLightBlendSingleChannelD(b) - b));
}

METAL_FUNC float4 softLightBlend(float4 Cb, float4 Cs)
{
    float4 B = float4(softLightBlendSingleChannel(Cb.r, Cs.r), softLightBlendSingleChannel(Cb.g, Cs.g),
                      softLightBlendSingleChannel(Cb.b, Cs.b), Cs.a);
    return blendBaseAlpha(Cb, Cs, B);
}
...
{
    float4 B = float4(colorBurnBlendSingleChannel(Cb.r, Cs.r), colorBurnBlendSingleChannel(Cb.g, Cs.g),
                      colorBurnBlendSingleChannel(Cb.b, Cs.b), Cs.a);
    return blendBaseAlpha(Cb, Cs, B);
}
...
{
    float4 B = float4(colorDodgeBlendSingleChannel(Cb.r, Cs.r), colorDodgeBlendSingleChannel(Cb.g, Cs.g),
                      colorDodgeBlendSingleChannel(Cb.b, Cs.b), Cs.a);
    return blendBaseAlpha(Cb, Cs, B);
}
...
{
    float4 B = float4(pinLightBlendSingleChannel(Cb.r, Cs.r), pinLightBlendSingleChannel(Cb.g, Cs.g),
                      pinLightBlendSingleChannel(Cb.b, Cs.b), Cs.a);
    return blendBaseAlpha(Cb, Cs, B);
}
...
{
    float4 B = float4(vividLightBlendSingleChannel(Cb.r, Cs.r), vividLightBlendSingleChannel(Cb.g, Cs.g),
                      vividLightBlendSingleChannel(Cb.b, Cs.b), Cs.a);
    return blendBaseAlpha(Cb, Cs, B);
}
...
{
    float4 B = float4(hardMixBlendSingleChannel(Cb.r, Cs.r), hardMixBlendSingleChannel(Cb.g, Cs.g),
                      hardMixBlendSingleChannel(Cb.b, Cs.b), Cs.a);
    return blendBaseAlpha(Cb, Cs, B);
}
...
{
    float4 B = float4(divideBlendSingleChannel(Cb.r, Cs.r), divideBlendSingleChannel(Cb.g, Cs.g),
                      divideBlendSingleChannel(Cb.b, Cs.b), Cs.a);
    return blendBaseAlpha(Cb, Cs, B);
}
...
METAL_FUNC float4 linearDodgeBlend(float4 Cb, float4 Cs) { return addBlend(Cb, Cs); }
...
METAL_FUNC float4 adjustVibrance(float4 colorInput, float vibrance, float3 grayColorTransform)
{
    ...
    color.rgb =
        mix(float3(luma), color.rgb,
            (1.0 + (vibrance *
                    (1.0 - (sign(vibrance) * color_saturation))))); // extrapolate between luma and original
                                                                    // by 1 + (1-saturation) - current
    // color.rgb = mix(vec3(luma), color.rgb, 1.0 + (1.0 -
    // pow(color_saturation, 1.0 - (1.0 - vibrance))) ); //pow version
...
METAL_FUNC float4 adjustSaturation(float4 textureColor, float saturation, float3 grayColorTransform)
{
    ...
    float3 rgbsat = max((pixel.rgb - gray) * gii, (gray - pixel.rgb) * gi);
    float sat = max(max(rgbsat.r, rgbsat.g), rgbsat.b);
    float skin = min(pixel.r - pixel.g, pixel.g * 2.0 - pixel.b) * 4.0 * (1.0 -
    rgbsat.r) * gi; skin = 0.15 + clamp(skin, 0.0, 1.0) * 0.7; float boost =
    ((sat * (sat - 1.0) + 1.0) * saturation) * (1.0-skin); pixel = clamp(pixel +
    (pixel - gray) * boost, 0.0, 1.0); pixel.a = textureColor.a; pixel.rgb +=
    pdelta.rgb; return pixel;
    */
...
METAL_FUNC float _continuousCornerSDF(float2 p, float dp)
{
    ...
    // Anti-aliasing. Manually calculate dfdx/dfdy here to avoid asymmetric
    // corners, because `fwdith` always calculates in one direction (x + 1, y +
    // 1)
...
METAL_FUNC float circularCornerMask(float2 canvasSize, float2 normalizedTextureCoordinate, float4 radius)
{
    ...
    {
        float2 p = float2(1.0 - textureCoordinate.x / radius[0], 1.0 - textureCoordinate.y / radius[0]);
        f[0] = _circularCornerSDF(p, 1 / radius[0]);
    }
    {
        float2 p = float2((textureCoordinate.x - rt.x) / radius[1], 1.0 - textureCoordinate.y / radius[1]);
        f[1] = _circularCornerSDF(p, 1 / radius[1]);
    }
    {
        float2 p = float2((textureCoordinate.x - rb.x) / radius[2], (textureCoordinate.y - rb.y) / radius[2]);
        f[2] = _circularCornerSDF(p, 1 / radius[2]);
    }
    {
        float2 p = float2(1.0 - textureCoordinate.x / radius[3], (textureCoordinate.y - lb.y) / radius[3]);
