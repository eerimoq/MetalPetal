...
float halftoneIntensity(float2 textureCoordinate,
                        float2 samplePos,
                        float scale,
                        float3 grayColorTransform,
...
float3 halftoneIntensityRGB(float2 textureCoordinate,
                            float2 samplePos,
                            float scale,
                            texture2d<float, access::sample> sourceTexture,
                            sampler sourceSampler)
...
float2 neighborSamplePosition(float2 textureCoordinate, float2 samplePosition, float scale, float2x2 m)
{
    ...
    float2 direction = (p.y > p.x) ? (-p.x > p.y ? float2(-1, 0) : float2(0, 1))
                                   : (-p.y > p.x ? float2(0, -1) : float2(1, 0));
                              ...
                              constant bool &singleAngleMode [[buffer(3)]])
{
    ...
    if (singleAngleMode) {
        ...
        intensityRGB =
            halftoneIntensityRGB(textureCoordinate, samplePos, scale, sourceTexture, customSampler);
        intensityNeighborRGB = halftoneIntensityRGB(
            textureCoordinate, neighborSamplePosition(textureCoordinate, samplePos, scale, m), scale,
            sourceTexture, customSampler);
    }
    else {
        ...
        intensityNeighborRGB =
            float3(halftoneIntensityRGB(textureCoordinate,
                                        ...
                                        scale, sourceTexture, customSampler)
                       .r,
                   halftoneIntensityRGB(textureCoordinate,
                                        neighborSamplePosition(textureCoordinate, samplePositionG, scale, mg),
                                        scale, sourceTexture, customSampler)
                       .g,
                   halftoneIntensityRGB(textureCoordinate,
                                        neighborSamplePosition(textureCoordinate, samplePositionB, scale, mb),
                                        scale, sourceTexture, customSampler)
                       .b);
    }
...
fragment float4 dotScreen(VertexOut vertexIn [[stage_in]],
                          texture2d<float, access::sample> sourceTexture [[texture(0)]],
                          sampler sourceSampler [[sampler(0)]],
                          ...
                          constant float3 &grayColorTransform [[buffer(2)]])
{
    ...
    float2 rotatedTextureCoordinate = m * textureCoordinate;
    float2 samplePos =
        (rotatedTextureCoordinate - mod(rotatedTextureCoordinate, float2(scale)) + scale * 0.5) * m;
    float intensity = halftoneIntensity(textureCoordinate, samplePos, scale, grayColorTransform,
                                        sourceTexture, customSampler);
    float2 samplePosNeighbor = neighborSamplePosition(textureCoordinate, samplePos, scale, m);
    float intensityNeighbor = halftoneIntensity(textureCoordinate, samplePosNeighbor, scale,
                                                grayColorTransform, sourceTexture, customSampler);
