...
METAL_FUNC half CLAHELookup(texture2d<half, access::sample> lutTexture,
                            sampler lutSamper,
                            float index,
                            float x)
{
...
fragment half4 CLAHEColorLookup(VertexOut vertexIn [[stage_in]],
                                ...
                                constant float2 &tileGridSize [[buffer(0)]])
{
    ...
    half res = (lutPlane1_ind1 * xa1_p + lutPlane1_ind2 * xa_p) * ya1 +
               (lutPlane2_ind1 * xa1_p + lutPlane2_ind2 * xa_p) * ya;
