...
vertex VertexOut passthroughVertex(const device VertexIn *vertices [[buffer(0)]], uint vid [[vertex_id]])
{
    VertexOut outVertex;
...
fragment float4 passthrough(VertexOut vertexIn [[stage_in]],
...
fragment float4 unpremultiplyAlpha(VertexOut vertexIn [[stage_in]],
                                   texture2d<float, access::sample> colorTexture [[texture(0)]],
                                   sampler colorSampler [[sampler(0)]])
{
...
fragment float4 unpremultiplyAlphaWithSRGBToLinearRGB(VertexOut vertexIn [[stage_in]],
...
fragment ColorAttachmentOneOutput
passthroughToColorAttachmentOne(VertexOut vertexIn [[stage_in]],
                                texture2d<float, access::sample> colorTexture [[texture(0)]],
                                sampler colorSampler [[sampler(0)]])
{
...
fragment ColorAttachmentOneOutput
unpremultiplyAlphaToColorAttachmentOne(VertexOut vertexIn [[stage_in]],
                                       texture2d<float, access::sample> colorTexture [[texture(0)]],
                                       sampler colorSampler [[sampler(0)]])
{
    ColorAttachmentOneOutput output;
...
fragment float4 premultiplyAlpha(VertexOut vertexIn [[stage_in]],
                                 texture2d<float, access::sample> colorTexture [[texture(0)]],
                                 sampler colorSampler [[sampler(0)]])
{
    float4 textureColor = colorTexture.sample(colorSampler, vertexIn.textureCoordinate);
...
fragment float4 convertITUR709RGBToLinearRGB(VertexOut vertexIn [[stage_in]],
                                             texture2d<float, access::sample> colorTexture [[texture(0)]],
                                             sampler colorSampler [[sampler(0)]])
{
    float4 textureColor = colorTexture.sample(colorSampler, vertexIn.textureCoordinate);
...
fragment float4 convertITUR709RGBToSRGB(VertexOut vertexIn [[stage_in]],
                                        texture2d<float, access::sample> colorTexture [[texture(0)]],
                                        sampler colorSampler [[sampler(0)]])
{
    float4 textureColor = colorTexture.sample(colorSampler, vertexIn.textureCoordinate);
...
fragment float4 convertLinearRGBToAppleLog(VertexOut vertexIn [[stage_in]],
                                           texture2d<float, access::sample> colorTexture [[texture(0)]],
                                           sampler colorSampler [[sampler(0)]])
{
    float4 textureColor = colorTexture.sample(colorSampler, vertexIn.textureCoordinate);
...
fragment float4 colorMatrixProjection(VertexOut vertexIn [[stage_in]],
                                      texture2d<float, access::sample> colorTexture [[texture(0)]],
                                      sampler colorSampler [[sampler(0)]],
                                      constant MTIColorMatrix &colorMatrix [[buffer(0)]])
{
    return colorTexture.sample(colorSampler, vertexIn.textureCoordinate) * colorMatrix.matrix +
           colorMatrix.bias;
}

fragment float4 colorLookup2DSquare(VertexOut vertexIn [[stage_in]],
...
fragment float4 colorLookup2DHorizontalStrip(VertexOut vertexIn [[stage_in]],
                                             texture2d<float, access::sample> sourceTexture [[texture(0)]],
                                             texture2d<float, access::sample> lutTexture [[texture(1)]],
                                             sampler colorSampler [[sampler(0)]],
                                             sampler lutSamper [[sampler(1)]],
...
fragment float4 colorLookup2DVerticalStrip(VertexOut vertexIn [[stage_in]],
                                           texture2d<float, access::sample> sourceTexture [[texture(0)]],
                                           texture2d<float, access::sample> lutTexture [[texture(1)]],
                                           sampler colorSampler [[sampler(0)]],
                                           sampler lutSamper [[sampler(1)]],
...
fragment float4 blendWithMask(VertexOut vertexIn [[stage_in]],
...
fragment float4 vibranceAdjust(VertexOut vertexIn [[stage_in]],
                               texture2d<float, access::sample> sourceTexture [[texture(0)]],
                               ...
                               constant float3 &grayColorTransform [[buffer(3)]])
{
    ...
    return amount > 0 ? (avoidsSaturatingSkinTones
                             ? adjustVibranceWhileKeepingSkinTones(textureColor, vibranceVector)
                             : adjustVibrance(textureColor, amount, grayColorTransform))
                      : adjustSaturation(textureColor, amount, grayColorTransform);
}

fragment float4 rToMonochrome(VertexOut vertexIn [[stage_in]],
                              texture2d<float, access::sample> sourceTexture [[texture(0)]],
                              sampler sourceSampler [[sampler(0)]],
...
fragment float4 rgToMonochrome(VertexOut vertexIn [[stage_in]],
                               texture2d<float, access::sample> sourceTexture [[texture(0)]],
                               sampler sourceSampler [[sampler(0)]],
...
fragment float4 chromaKeyBlend(VertexOut vertexIn [[stage_in]],
                               texture2d<float, access::sample> sourceTexture [[texture(0)]],
                               texture2d<float, access::sample> backgroundTexture [[texture(1)]],
                               sampler sourceSampler [[sampler(0)]],
...
fragment float4 crt(VertexOut vertexIn [[stage_in]],
                    texture2d<float, access::sample> sourceTexture [[texture(0)]],
                    sampler sourceSampler [[sampler(0)]],
...
fragment float4 rgbToneCurveAdjust(VertexOut vertexIn [[stage_in]],
                                   texture2d<float, access::sample> sourceTexture [[texture(0)]],
                                   ...
                                   constant float &intensity [[buffer(0)]])
{
    float4 textureColor = sourceTexture.sample(sourceSampler, vertexIn.textureCoordinate);
    float r =
        toneCurveTexture.sample(toneCurveSampler, float2((textureColor.r * 255.0 + 0.5) / 256.0, 0.5)).r;
    float g =
        toneCurveTexture.sample(toneCurveSampler, float2((textureColor.g * 255.0 + 0.5) / 256.0, 0.5)).g;
    float b =
        toneCurveTexture.sample(toneCurveSampler, float2((textureColor.b * 255.0 + 0.5) / 256.0, 0.5)).b;
...
fragment float4 usmSecondPass(VertexOut vertexIn [[stage_in]],
                              texture2d<float, access::sample> sourceTexture [[texture(0)]],
...
kernel void histogramDisplayFindMax(texture2d<uint, access::read> sourceTexture [[texture(0)]],
...
/*
fragment float4 histogramDisplay(VertexOut vertexIn [[stage_in]],
                                 texture2d<uint, access::sample> sourceTexture
[[texture(0)]], sampler sourceSampler [[sampler(0)]], texture2d<uint,
access::read> maxValueTexture [[texture(1)]]) { uint rValue =
sourceTexture.sample(sourceSampler, float2(vertexIn.textureCoordinate.x,
0.5/4.0)).r; uint gValue = sourceTexture.sample(sourceSampler,
float2(vertexIn.textureCoordinate.x, 1.5/4.0)).r; uint bValue =
sourceTexture.sample(sourceSampler,
float2(vertexIn.textureCoordinate.x, 2.5/4.0)).r; uint4 maxValue =
maxValueTexture.read(uint2(0,0)); float3 height = float3(rValue, gValue,
bValue)/float3(max(max(maxValue.r, maxValue.g),maxValue.b)); bool3 fill =
vertexIn.textureCoordinate.y > 1.0 - height; return float4(float3(fill), 1.0);
...
fragment float4 roundCorner_programmableBlending(VertexOut vertexIn
[[stage_in]], constant float & radius [[buffer(0)]], constant float2 & center
[[buffer(1)]], float4 currentColor [[ color(0) ]]) {
    //4xAA
                               ...
                               sampler sourceSampler [[sampler(0)]])
{
    ...
    result.a *= circularCornerMask(float2(sourceTexture.get_width(), sourceTexture.get_height()),
                                   vertexIn.textureCoordinate, radius);
    return result;
}
...
{
    float4 result = sourceTexture.sample(sourceSampler, vertexIn.textureCoordinate);
    result.a *= continuousCornerMask(float2(sourceTexture.get_width(), sourceTexture.get_height()),
                                     vertexIn.textureCoordinate, radius);
    return result;
}
} // namespace metalpetal
