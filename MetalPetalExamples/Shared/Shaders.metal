...
{
    return float4(float3(1.0 - smoothstep(0.3, 0.8, distance(vertexIn.textureCoordinate, float2(0.5, 0.5)))),
                  1);
}
...
fragment float4 imageConvolution(VertexOut vertexIn [[stage_in]],
                                 texture2d<float, access::sample> sourceTexture [[texture(0)]],
                                 sampler sourceSampler [[sampler(0)]],
    ...
    if (currentRadius == 0) {
        ...
        for (int offsetX = -currentRadius; offsetX < currentRadius; offsetX += 1) {
            for (int offsetY = -currentRadius; offsetY < currentRadius; offsetY += 1) {
                float2 sampleOffset =
                    float2(offsetX, offsetY) / float2(sourceTexture.get_width(), sourceTexture.get_height());
                float maskValue =
                    1.0 - maskTexture.sample(maskSampler, vertexIn.textureCoordinate + sampleOffset).r;
                float kernelValue =
                    kernelTexture
                        .sample(kernelSampler, float2((offsetX + currentRadius) / currentRadius / 2,
                                                      1.0 - (offsetY + currentRadius) / currentRadius / 2))
                        .r *
                    maskValue;
                color += sourceTexture.sample(sourceSampler, vertexIn.textureCoordinate + sampleOffset).rgb *
                         kernelValue;
...
vertex BouncingBallVertex bouncingBallVertex(const device ParticleData *data [[buffer(0)]],
                                             uint vid [[vertex_id]],
...
fragment float4 bouncingBallFragment(BouncingBallVertex vertexIn [[stage_in]],
