...
struct VideoProcessorView: View {
    class VideoProcessor: ObservableObject {
        ...
        func updateVideoURL(_ url: URL) async throws {
            ...
            let mask = DemoImages.makeSymbolImage(
                named: "seal.fill",
                aspectFitIn: whiteSquare.size,
                padding: whiteSquare.size.width / 10
            )
            ...
            let videoComposition = try await MTIVideoComposition(
                ...
                filter: { request in
                    ...
                    watermarkMaskTransformFilter.transform = CATransform3DMakeRotation(
                        CGFloat(request.compositionTime.seconds),
                        0,
                        0,
                        1
                    )
                    let pixellateScale = mix(
                        SIMD2<Float>(50, 50),
                        SIMD2<Float>(1, 1),
        ...
        func export(completion: @escaping (Result<URL, Error>) -> Void) {
            guard let asset = videoAsset, let videoComposition else {
                return
            }
            exportProgress = nil
            ...
            var configuration = AssetExportSession.Configuration(
                fileType: .mp4,
                videoSettings: .h264(videoSize: videoComposition.renderSize),
            ...
            do {
                let exportSession = try AssetExportSession(
                    asset: asset,
                    outputURL: outputURL,
