...
struct BokehEffectView: View {
    ...
    enum BokehShape: Hashable, Identifiable {
        ...
        static let all: [BokehShape] = [
            ...
            .custom(
                "Heart",
                DemoImages.makeSymbolAlphaMaskImage(
                    named: "suit.heart.fill",
                    aspectFitIn: CGSize(width: 64, height: 64)
                )
            ),
            .custom(
                "Star",
    ...
    private func applyFilter() throws -> CGImage {
        ...
        case .hexagon:
            ...
            bokeh.inputImage = inputImage
                .withSamplerDescriptor(.defaultSamplerDescriptor(withAddressMode: .clampToEdge))
        ...
        case let .custom(_, kernelImage):
            outputImage = CustomShapeBokehWithMask.bokeh(
                image: inputImage,
                mask: inputDepthMask,
        ...
        return try imageRenderer.render(outputImage, using: renderContext).cgImage
    }
    ...
    var body: some View {
        ...
        .noteOverlay(NoteView {
            VStack(alignment: .leading, spacing: 6) {
                Text(
                    "This is an example of applying various bokeh effects to the input image with a depth mask."
                )
                HStack {
                    VStack {
                        Image(cgImage: DemoImages.cgImage(named: "P1.jpg")).resizable()
                            .aspectRatio(contentMode: .fit)
                        Text("Image").font(Font.caption).foregroundColor(.secondary)
                    }
                    VStack {
                        Image(cgImage: DemoImages.cgImage(named: "P1_depth.jpg")).resizable()
                            .aspectRatio(contentMode: .fit)
                        Text("Depth Mask").font(Font.caption).foregroundColor(.secondary)
                    }
                }.frame(height: 128)
                Text(
                    "The hexagon shape bokeh blur effect comes from a bulit-in filter - MTIHexagonalBokehBlurFilter."
                )
                Button("Hexagonal Bokeh Blur") { [openURL] in
                    openURL(URL(string: "https://github.com/YuAo/HexagonalBokehBlur")!)
                }.linkButtonStyle()
                Text("""
                Other bokeh effects are implemented in the example project \
                using straight forward image convolution.
                """)
...
enum CustomShapeBokehWithMask {
    private static let powKernel = MTIRenderPipelineKernel(
        vertexFunctionDescriptor: .passthroughVertex,
        fragmentFunctionDescriptor: MTIFunctionDescriptor(name: "imagePow", in: .main)
    )
    ...
    private static let convolutionKernel = MTIRenderPipelineKernel(
        vertexFunctionDescriptor: .passthroughVertex,
        fragmentFunctionDescriptor: MTIFunctionDescriptor(name: "imageConvolution", in: .main)
    )
    private static func applyImageConvolution(
        to image: MTIImage,
        mask: MTIImage,
        kernelSize: Int,
        kernelImage: MTIImage,
    ...
    static func bokeh(
        ...
        let bokehImage = applyImageConvolution(
            to: powered,
            mask: mask,
        ...
        return pow(bokehImage, 1.0 / power)
    }
}
