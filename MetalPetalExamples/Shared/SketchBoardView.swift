...
struct SketchBoardView: View {
    ...
    class Renderer: ObservableObject {
        ...
        func updateCanvasSize(_ size: CGSize) {
            ...
            if let imageBuffer = previousImageBuffer,
               canvasSize.width > backgroundImage.size.width || canvasSize.height > backgroundImage.size
               .height
            {
                backgroundImage = MTIImage(
                    ...
                    sRGB: false,
                    size: CGSize(
                        width: max(backgroundImage.size.width, canvasSize.width),
        ...
        private func updateBrushImage() {
            ...
            guard let context = CGContext(
                data: nil,
                width: Int(pixelSize) + 2,
                height: Int(pixelSize) + 2,
                bitsPerComponent: 8,
                bytesPerRow: 0,
            ...
            let brushImage = MTIImage(cgImage: context.makeImage()!, isOpaque: false).premultiplyingAlpha()
        ...
        private func draw(at point: CGPoint) {
            ...
            if d > 1 {
                for i in 1 ..< Int(d) {
                    let p = quadBezierPoint(
                        t: CGFloat(i) / CGFloat(d),
                        start: mid1,
                        c1: previousLocation,
                        end: mid2
                    )
                    ...
                    case .dynamic:
                        MTIColor(
                            red: Float(p.x) / Float(backgroundImage.size.width),
                            green: Float(p.y) / Float(backgroundImage.size.height),
                            blue: 1,
                            alpha: 1
                        )
                    case let .static(color):
                        MTIColor(red: color.r, green: color.g, blue: color.b, alpha: color.a)
                    case .eraser:
                        backgroundColor
                    }
                ...
                case .dynamic:
                    MTIColor(
                        red: Float(point.x) / Float(backgroundImage.size.width),
                        green: Float(point.y) / Float(backgroundImage.size.height),
                        blue: 1,
                        alpha: 1
                    )
                case let .static(color):
        ...
        private func output(image: MTIImage) {
            do {
                ...
                _ = try renderContext.startTask(toRender: image, completion: nil)
                ...
                self.image = try imageRenderer.render(croppedImage, using: renderContext).cgImage
            } catch {
                print(error)
            }
        }
            ...
            VStack {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    HStack {
                        ...
                        BrushColorButton(
                            $renderer.brushColor,
                            color: .static(BrushColor.Color(r: 1, g: 203 / 255.0, b: 0, a: 1))
                        )
                        BrushColorButton(
                            $renderer.brushColor,
                            color: .static(BrushColor.Color(r: 0, g: 203 / 255.0, b: 71 / 255.0, a: 1))
                        )
                        BrushColorButton(
                            $renderer.brushColor,
                            color: .static(BrushColor.Color(r: 0, g: 203 / 255.0, b: 1, a: 1))
                        )
                        BrushColorButton(
                            $renderer.brushColor,
                            color: .static(BrushColor.Color(r: 1, g: 0, b: 79 / 255.0, a: 1))
                        )
                        BrushColorButton(
                            $renderer.brushColor,
                            color: .static(BrushColor.Color(r: 0, g: 0, b: 0, a: 1))
                        )
    ...
    struct BrushColorButton: View {
        ...
        static let dynamicBrushIcon: CGImage = RGUVB1GradientImage.makeCGImage(size: CGSize(
            width: BrushColorButton.preferredSize * 2,
            height: BrushColorButton.preferredSize * 2
        ))
        ...
        init(_ value: Binding<BrushColor>, color: BrushColor) {
            self.value = value
            self.color = color
            isSelected = value.wrappedValue == color
        }
        ...
        var body: some View {
            Group {
                ...
                case .eraser:
                    Image(systemName: "square.tophalf.fill").resizable().padding(10).background(Color(
                        .sRGB,
                        white: 1,
                        opacity: 1
                    ))
...
struct TouchTrackingView: ViewRepresentable {
    ...
    init(
        touchBeganHandler: @escaping TouchHandler,
        touchMovedHandler: @escaping TouchHandler,
        boundsChangedHandler: @escaping BoundsChangeHandler
    ) {
        touchBeginHandler = touchBeganHandler
        self.touchMovedHandler = touchMovedHandler
        self.boundsChangedHandler = boundsChangedHandler
    }

    func makeUIView(context _: Context) -> TouchTrackingNativeView {
        let view = TouchTrackingNativeView(frame: .zero)
        view.touchBeginHandler = touchBeginHandler
        view.touchMovedHandler = touchMovedHandler
        view.boundsChangedHandler = boundsChangedHandler
        return view
    }

    func updateUIView(_: TouchTrackingNativeView, context _: Context) {}
