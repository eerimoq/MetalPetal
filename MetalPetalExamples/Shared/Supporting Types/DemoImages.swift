...
enum DemoImages {
    ...
    static let p1040808 = MTIImage(
        contentsOf: Bundle.main.url(forResource: "P1040808", withExtension: "jpg")!,
        isOpaque: true
    )!

    static let p1 = MTIImage(
        contentsOf: Bundle.main.url(forResource: "P1", withExtension: "jpg")!,
        isOpaque: true
    )!
    static let p1DepthMask = MTIImage(
        contentsOf: Bundle.main.url(forResource: "P1_depth", withExtension: "jpg")!,
        isOpaque: true
    )!

    static let colorLookupTable = MTIImage(
        contentsOf: Bundle.main.url(forResource: "ColorLookupTable", withExtension: "png")!,
        isOpaque: true
    )!

    static func makeSymbolImage(named name: String, aspectFitIn size: CGSize,
                                padding: CGFloat = 0) -> MTIImage
    {
        #if os(iOS)
        guard let cgImage = UIImage(
            systemName: name,
            withConfiguration: UIImage.SymbolConfiguration(
                pointSize: min(size.width, size.height),
                weight: .medium
            )
        )?.cgImage else {
            fatalError()
        ...
        #elseif os(macOS)
        guard let cgImage = NSImage(systemSymbolName: name, accessibilityDescription: nil)?
        ...
        context.draw(
            ...
            in: MTIMakeRect(
                ...
                insideRect: CGRect(origin: .zero, size: size).insetBy(dx: padding, dy: padding)
            )
        )
    ...
    static func makeSymbolAlphaMaskImage(named name: String, aspectFitIn size: CGSize,
                                         padding: CGFloat = 0) -> MTIImage
    {
        #if os(iOS)
        guard let cgImage = UIImage(
...
enum RGUVB1GradientImage {
    ...
    static func makeCGImage(size: CGSize) -> CGImage {
        ...
        return try! context.makeCGImage(from: makeImage(size: size))
    }
}

enum RadialGradientImage {
    private static let kernel = MTIRenderPipelineKernel(
        vertexFunctionDescriptor: .passthroughVertex,
