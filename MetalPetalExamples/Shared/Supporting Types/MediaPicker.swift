...
struct VideoPicker: View {
    ...
    init(title: some StringProtocol, handler: @escaping (URL) -> Void) {
    ...
    var body: some View {
        ...
        return Button(title, action: { [handler] in
            if let viewController = UIApplication.shared.topMostViewController {
                ...
                videoPickerDelegateHolder.videoPickerDelegate = delegate
    ...
    var body: some View {
        ...
        return Button(title, action: { [handler] in
               ...
               let viewController = rootWindow.rootViewController
            {
                ...
                picker.delegate = delegate
                pickerDelegateHolder.pickerDelegate = delegate
                viewController.present(picker, animated: true, completion: nil)
            }
        })
        #elseif os(macOS)
