...
struct ImageFilterView<Filter: MTIFilter>: View {
    ...
    init(
        filter: Filter,
        filterInputKeyPath: ReferenceWritableKeyPath<Filter, MTIImage?>,
        parameters: [FilterParameter<Filter>],
        isChangingImageAllowed: Bool = true
    ) {
    ...
    private func valueBinding(for parameter: FilterParameter<some Any>) -> Binding<Float> {
        Binding<Float>(get: {
                    ...
                    VStack {
                        ForEach(parameters) { parameter in
                            VStack(alignment: .leading) {
                                let value = values[parameter.name, default: parameter.defaultValue]
                                Text("\(parameter.name) \(value, specifier: "%.2f")")
                                if let step = parameter.step {
                                    Slider(
                                        value: valueBinding(for: parameter),
                                        in: parameter.sliderRange,
                                        step: step,
